...
(function (OfficeExtension) {
    ...
    (function (WacRuntime) {
        ...
        (function (ApiFlags) {
            ...
            ApiFlags[ApiFlags["copilotSafe"] = 4096] = "copilotSafe";
...
(function (OfficeExtension) {
    ...
    (function (WacRuntime) {
        var Constants = (function () {
            ...
            Constants.performanceMetadata = "PerformanceMetadata";
    ...
    (function (WacRuntime) {
        var JsComRequestProcessor = (function () {
            function JsComRequestProcessor(context) {
                ...
                this.m_objectPaths = {};
                this.m_perfLogger = new WacRuntime.PerfLogger();
                this.m_resolvedObjects = {};
                this.m_traceIds = [];
                this.m_objectPathIdToSafeForCellEditModeMap = {};
            }
            JsComRequestProcessor.prototype.process = function () {
                ...
                var isPerformanceLoggingEnabled = this.isChangeGateEnabled("Microsoft.Office.SharedOnline.ChangeGate.RichApiExtendedLogging");
                for (var i = 0; i < this.m_requestMessage.Actions.length; i++) {
                    ...
                    if (isPerformanceLoggingEnabled) {
                        var startMarkFunc = this.startMark(this.m_requestMessage.Actions[i].Id.toString());
                        ret = ret.then(startMarkFunc);
                    }
                    ret = ret.then(func);
                    var func2 = this.createWriteActionWriterFunc(this.m_context.response.writer, actionWriter);
                    ret = ret.then(func2);
                    if (isPerformanceLoggingEnabled) {
                        var endMarkFunc = this.endMark(this.m_requestMessage.Actions[i].Id.toString());
                        ret = ret.then(endMarkFunc);
                    }
                }
                return ret.then(function () {
                    if (isPerformanceLoggingEnabled) {
                        _this.writePerformanceMetrics();
                    }
            ...
            JsComRequestProcessor.prototype.startMark = function (actionIndex) {
                var _this = this;
                return function () {
                    return WacRuntime.Utility.createPromiseFromResult(null)
                        .then(function () {
                        _this.m_perfLogger.startMark(actionIndex);
                    });
                };
            };
            ;
            JsComRequestProcessor.prototype.endMark = function (actionIndex) {
                var _this = this;
                return function () {
                    return WacRuntime.Utility.createPromiseFromResult(null)
                        .then(function () {
                        _this.m_perfLogger.endMark(actionIndex);
                    });
                };
            };
            ;
            JsComRequestProcessor.prototype.writePerformanceMetrics = function () {
                var perfEntries = this.m_perfLogger.getEntries();
                if (perfEntries.length === 0) {
                    return;
                }
                var writer = this.m_context.response.writer;
                writer.startObjectScope();
                writer.writeName(WacRuntime.Constants.performanceMetadata);
                writer.startArrayScope();
                for (var i = 0; i < perfEntries.length; i++) {
                    writer.startObjectScope();
                    writer.writeName(WacRuntime.Constants.actionId);
                    writer.writeValue(perfEntries[i].name);
                    writer.writeName(WacRuntime.Constants.latency);
                    writer.writeValue(perfEntries[i].latency);
                    writer.endScope();
                }
                writer.endScope();
                writer.endScope();
                this.m_perfLogger.clear();
            };
            JsComRequestProcessor.prototype.isChangeGateEnabled = function (changeGateName) {
                if (window.OSF && window.OSF.DisabledChangeGates) {
                    var disabledChangeGates = window.OSF.DisabledChangeGates;
                    return disabledChangeGates.indexOf(changeGateName) === -1;
                }
                return true;
            };
            ...
            JsComRequestProcessor.prototype.preProcessIsSafeForCellEditMode = function () {
                if (this.m_requestMessage && this.m_requestMessage.Actions) {
                    for (var i = 0; i < this.m_requestMessage.Actions.length; i++) {
                        if (!this.isSafeForCellEditModeAction(this.m_requestMessage.Actions[i])) {
                            return false;
                        }
                    }
                }
            ...
            JsComRequestProcessor.prototype.isSafeForCellEditModeAction = function (action) {
                switch (action.ActionType) {
                    case 5:
                        return true;
                    case 1:
                        return this.isGlobalObject(action.ObjectPathId) || this.isSafeForCellEditModeObjectPath(action.ObjectPathId);
                    case 2:
                    case 4:
                    case 6:
                    case 7:
                    case 8:
                    case 9:
                        return this.isSafeForCellEditModeObjectPath(action.ObjectPathId);
                    case 3:
                        return this.isSafeForCellEditModeObjectPath(action.ObjectPathId)
                            || this.isSafeForCellEditModeGlobalObjectMethod(action.ObjectPathId, action.Name);
                    default:
                        return false;
                }
            };
            JsComRequestProcessor.prototype.isGlobalObject = function (objectPathId) {
                if (objectPathId in this.m_requestMessage.ObjectPaths) {
                    var objectPathInfo = this.m_requestMessage.ObjectPaths[objectPathId];
                    return objectPathInfo.ObjectPathType === 1;
                }
                var richEx = new WacRuntime.RichApiException(WacRuntime.ErrorCodes.invalidArgument, WacRuntime.Utility.getLocalizedString(WacRuntime.ResourceStrings.invalidArgument, "objectPath"));
                richEx.location = "isGlobalObject";
                throw richEx;
            };
            JsComRequestProcessor.prototype.isSafeForCellEditModeObjectPath = function (objectPathId) {
                if (objectPathId in this.m_objectPathIdToSafeForCellEditModeMap) {
                    return this.m_objectPathIdToSafeForCellEditModeMap[objectPathId];
                }
                var isSafe = false;
                if (objectPathId in this.m_requestMessage.ObjectPaths) {
                    var objectPathInfo = this.m_requestMessage.ObjectPaths[objectPathId];
                    if (objectPathInfo.ObjectPathType === 2) {
                        var typeName = objectPathInfo.Name;
                        if (this.m_context.typeReg.getType(typeName).typeFlags & WacRuntime.TypeFlags.safeForCellEditMode) {
                            isSafe = true;
                        }
                    }
                    else if (objectPathInfo.ObjectPathType === 7) {
                        isSafe = true;
                    }
                    else if (objectPathInfo.ParentObjectPathId > 0) {
                        isSafe = this.isSafeForCellEditModeObjectPath(objectPathInfo.ParentObjectPathId);
                    }
                    this.m_objectPathIdToSafeForCellEditModeMap[objectPathId] = isSafe;
                    return isSafe;
                }
                var richEx = new WacRuntime.RichApiException(WacRuntime.ErrorCodes.invalidArgument, WacRuntime.Utility.getLocalizedString(WacRuntime.ResourceStrings.invalidArgument, "objectPath"));
                richEx.location = "isSafeForCellEditModeObjectPath";
                throw richEx;
            };
            JsComRequestProcessor.prototype.isSafeForCellEditModeGlobalObjectMethod = function (objectPathId, actionName) {
                if (this.isGlobalObject(objectPathId)) {
                    return actionName === WacRuntime.Constants.removeReference
                        || actionName === WacRuntime.Constants.removeAllReferences
                        || actionName === WacRuntime.Constants.getReferenceCount;
                }
                return false;
            };
...
(function (OfficeExtension) {
    ...
    (function (WacRuntime) {
        var PerfLogger = (function () {
            ...
            PerfLogger.prototype.startMark = function (name) {
                var start = this.getTimestamp();
                this.m_active[name] = { name: name, start: start };
            };
            PerfLogger.prototype.endMark = function (name) {
                var mark = this.m_active[name];
                if (!mark) {
                    return;
                }
                var end = this.getTimestamp();
                mark.end = end;
                mark.latency = end - mark.start;
                this.m_entries.push(mark);
                delete this.m_active[name];
            };
            PerfLogger.prototype.getEntries = function () {
                return this.m_entries.slice();
            };
            PerfLogger.prototype.clear = function () {
                this.m_active = {};
                this.m_entries = [];
            };
            PerfLogger.prototype.getTimestamp = function () {
                return (typeof performance !== "undefined" && typeof performance.now === "function") ? performance.now() : Date.now();
            };
            return PerfLogger;
        }());
        WacRuntime.PerfLogger = PerfLogger;
    })(WacRuntime = OfficeExtension.WacRuntime || (OfficeExtension.WacRuntime = {}));
})(OfficeExtension || (OfficeExtension = {}));
var OfficeExtension;
(function (OfficeExtension) {
...
(function (OfficeExtension) {
    ...
    (function (WacRuntime) {
        var RichApi = (function () {
            ...
            RichApi.ensureTypeRegistration = function (typeRegFunc) {
            ...
            RichApi.preProcessIsSafeForCellEditMode = function (typeRegFunc, request) {
                RichApi.ensureTypeRegistration(typeRegFunc);
                var context = new WacRuntime.RichApiRequestContext(RichApi.s_typeReg, null);
                context.request.init(request, null);
                var isSafe = false;
                try {
                    if (context.request.requestType === 0) {
                        var processor = new WacRuntime.JsComRequestProcessor(context);
                        isSafe = processor.preProcessIsSafeForCellEditMode();
                    }
                }
                catch (ex) {
                    isSafe = false;
...
(function (OfficeExtension) {
    ...
    (function (WacRuntime) {
        ...
        (function (RichApiRequestFlags) {
            ...
            RichApiRequestFlags[RichApiRequestFlags["AllowImmediateExecution"] = 8192] = "AllowImmediateExecution";
            RichApiRequestFlags[RichApiRequestFlags["SetInvocationResultsOperation"] = 16384] = "SetInvocationResultsOperation";
            RichApiRequestFlags[RichApiRequestFlags["OnlyAllowCopilotSafeApi"] = 32768] = "OnlyAllowCopilotSafeApi";
...
(function (OfficeExtension) {
    ...
    (function (WacRuntime) {
        ...
        (function (TypeFlags) {
            ...
            TypeFlags[TypeFlags["doPostProcessAfterPatch"] = 128] = "doPostProcessAfterPatch";
            TypeFlags[TypeFlags["copilotSafe"] = 256] = "copilotSafe";
        ...
        var UtilityInternal = (function () {
            ...
            UtilityInternal.checkCreateObjectPermission = function (requestContext, typeInfo) {
                if (typeInfo.typeCategory === WacRuntime.TypeCategory.classObject) {
                    ...
                    UtilityInternal.checkSafeTypeFlags(requestContext, typeInfo.typeFlags, typeInfo.name);
                }
            };
            UtilityInternal.checkPropertyGetPermission = function (requestContext, typeInfo, propInfo) {
                if (typeInfo.typeCategory === WacRuntime.TypeCategory.classObject &&
                    ...
                    UtilityInternal.checkSafeApiFlags(requestContext, propInfo.apiFlags, propInfo.name, propInfo.apiName);
                }
            };
            ...
            UtilityInternal.checkMethodPermission = function (requestContext, typeInfo, methodInfo) {
                if (typeInfo.typeCategory === WacRuntime.TypeCategory.classObject) {
                    ...
                    UtilityInternal.checkSafeApiFlags(requestContext, methodInfo.apiFlags, methodInfo.name, methodInfo.apiName);
                }
            };
            UtilityInternal.checkSafeTypeFlags = function (requestContext, typeFlags, apiName) {
                ...
                if ((requestContext.request.requestFlags & WacRuntime.RichApiRequestFlags.OnlyAllowCopilotSafeApi) !== 0 &&
                    (typeFlags & WacRuntime.TypeFlags.copilotSafe) === 0) {
                    throw UtilityInternal.createAccessDeniedException(apiName);
                }
            };
            ...
            UtilityInternal.checkSafeApiFlags = function (requestContext, apiFlags, name, apiName) {
                if ((requestContext.request.requestFlags & WacRuntime.RichApiRequestFlags.OnlyAllowMakerSafeApi) !== 0 &&
                ...
                if ((requestContext.request.requestFlags & WacRuntime.RichApiRequestFlags.OnlyAllowCopilotSafeApi) !== 0 &&
                    (apiFlags & WacRuntime.ApiFlags.copilotSafe) === 0) {
                    if (!UtilityInternal.isSpecialApiNotRequirePermissionCheck(name)) {
                        throw UtilityInternal.createAccessDeniedException(apiName);
                    }
                }
