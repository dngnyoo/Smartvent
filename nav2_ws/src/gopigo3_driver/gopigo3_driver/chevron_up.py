#!/usr/bin/env python3
"""
Chevron Display - up Direction
Standalone module for displaying up-pointing chevrons moving up.

Usage:
    python chevron_up.py
    
Or import and use:
    from chevron_up import display
    display()
"""
...
def display(speed: float = 220.0, spacing: float = 220.0, duration: float = None):
    """
    Display up-pointing chevrons moving up.
    ...
    pygame.display.set_caption("Chevron Display - up")
    ...
    while running:
        ...
        # Update animation offset (moving up)
        offset += speed * dt
        if offset >= spacing:
            offset -= spacing
        ...
        # Draw vertical stream of up-pointing chevrons
        y = screen_height / 2
        count = int(screen_width / spacing) + 3
        
        for i in range(-1, count):
            x = i * spacing - offset  # Moving up (negative offset)
