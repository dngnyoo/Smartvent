#!/usr/bin/env python3
"""
Chevron Display - down Direction
Standalone module for displaying down-pointing chevrons moving down.

Usage:
    python chevron_down.py
    
Or import and use:
    from chevron_down import display
    display()
"""
...
def display(speed: float = 220.0, spacing: float = 220.0, duration: float = None):
    """
    Display down-pointing chevrons moving down.
    ...
    pygame.display.set_caption("Chevron Display - down")
    ...
    while running:
        ...
        # Update animation offset (moving down)
        offset += speed * dt
        if offset >= spacing:
            offset -= spacing
        ...
        # Draw vertical stream of down-pointing chevrons
        y = screen_height / 2
        count = int(screen_width / spacing) + 3
        
        for i in range(-1, count):
            x = i * spacing + offset  # Moving down
            
            if -spacing <= x <= screen_width + spacing:
